...
function repeatStringNumTimes(param1, param2) {
	let result_string = '';
	if (param2 <= 0) {
		return result_string;
	} else {
		for (let i = param2; i > 0; i--) {
			result_string += param1;
		}
...
function repeatStringNumTimesHandler() {
	...
	// read user input
	const input_first = prompt('What is the name of Animal');
	const input_second = prompt('have many Kangaroos do you want to see?');
	const cast_second_input = Number(input_second);

	// core logic
	const result = repeatStringNumTimes(input_first, cast_second_input);
	...
	// log for developers
	console.log('first input: ', input_first, `\n type: ${typeof input_first}`);
	console.log('Second input: ', input_second, `\n type: ${typeof input_second}`);
	console.log('convert to number', cast_second_input, `\n type: ${typeof cast_second_input}`);
	console.log('Result of input for user: ', result, `\n type: ${typeof result}`);
}
